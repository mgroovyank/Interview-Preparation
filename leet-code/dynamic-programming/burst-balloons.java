...
class Solution {
    public int maxCoins(int[] nums) {
        ...
        return maxCoinsPartition(newNums, 1, n-2, dp);
    }

    private int maxCoinsPartition(int[] newNums, int i, int j, int[][] dp){
        // base case
        if(i>j){
            return 0;
        }
        ...
        // choose kth indexed ballon to be bursted last in the range
        for(int k=i;k<=j;k++){
            maxCoins = Math.max(maxCoins, newNums[i-1]*newNums[k]*newNums[j+1] + maxCoinsPartition(newNums, i, k-1, dp)
                + maxCoinsPartition(newNums, k+1, j, dp));
        }
    ...
    public int maxCoins(int[] nums) {
        ...
        for(int i=n-2;i>=1;i--){
            for(int j=i;j<=n-2;j++){
                ...
                // choose kth indexed ballon to be bursted last in the range
                for(int k=i;k<=j;k++){
                    maxCoins = Math.max(maxCoins, newNums[i-1]*newNums[k]*newNums[j+1] + dp[i][k-1] + dp[k+1][j]);
                }
                dp[i][j] = maxCoins;
            }
        }
